...
#################################################
score_path = "/group/diangelantonio/users/alessia_mapelli/Brain_simulations/Model A/fpc_score_50_balanced.csv"
grouping_path = "/group/diangelantonio/users/alessia_mapelli/Brain_simulations/Model A/true_grouping_reverse_50.csv"
output_path = "/group/diangelantonio/users/alessia_mapelli/Brain_simulations/Model A/sample_size_study/Screening_procedure/50_balanced/"
...
if (pre_screen == TRUE) {cat("The pre-screening procedure is active \n")}
#################################################
...
if (pre_screen) {
  ...
  protdata_matrix <- as.matrix(scores)
  Scor <- cor(protdata_matrix)
  if (verbose) { cat("Done computing correlation matrix \n") }
  if (is.null(pre_screen_threshold)) { pre_screen_threshold <- quantile(abs(Scor), probs = 0.2) }
  scr_index <- rep(FALSE, p)
  for(it in 1:p){
      if(it != j && sum(abs(Scor[(j-1)*M + (1:M), (it-1)*M + (1:M)])>pre_screen_threshold)>0){
        scr_index[it] <- TRUE
      }
  }
}
