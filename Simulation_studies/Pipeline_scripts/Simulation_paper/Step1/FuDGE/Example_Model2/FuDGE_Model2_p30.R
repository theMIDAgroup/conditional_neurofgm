...
save.path <- paste0(model.path, "/result")
...
# FROM HERE ON WE COULD USE OUR DATA
...
ROC.result <- matrix(0, nrow=length(lambda.v), ncol=2)
colnames(ROC.result) <- c("TPR", "FPR")
Delta.initial <- matrix(0, nrow=(p*M.selected), ncol=(p*M.selected))
for (lambda.ind in c(1:length(lambda.v)) ){
  lambda.choose <- lambda.v[lambda.ind]
  # Set the matrix representing true differential edges
  Supp.Mat <- Theta.L$SupportDelt
  
  # Compute the differential graph
  DFGM_g <- ProxAlg_DFGM(estimated.cov.pc.X, estimated.cov.pc.Y, p, M.selected, 
  ...
  for (i in 1:(p - 1)){
    ...
    v.true <- c(v.true, Supp.Mat[i, (i + 1):p])
  }
  ...
  ROC.result[lambda.ind, ] <- c(TPR, FPR)
}


# TAke OUT Edgeshat to get the adj metrix

Paral.result <- ROC.result

save(Paral.result, file=paste(save.path, "/Para_result_p", p, "_runind", 
                            run.ind, ".RData", sep=""))